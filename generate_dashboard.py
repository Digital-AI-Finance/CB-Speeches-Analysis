...
Usage:
    python generate_dashboard.py                    # Output to dashboard.html
    python generate_dashboard.py --output docs/     # Output to docs/dashboard.html
"""
import argparse
import pandas as pd
import json
...
def load_data_summary() -> dict:
    ...
    try:
        ...
        summary['pca_components'] = {'rows': len(df), 'columns': list(df.columns)}
    except Exception as e:
    ...
    try:
        ...
        summary['sentiment'] = {'rows': len(df), 'columns': list(df.columns)}
    except Exception as e:
...
def generate_html(data_summary: dict, use_relative_paths: bool = False) -> str:
    ...
    # Image path prefix
    img_prefix = "figures/" if use_relative_paths else str(FIGURES_DIR) + "/"
...
'''
    ...
    for folder, title in CHART_FOLDERS:
        png_filename = f"{folder}.png"
        html += f'''
                    <div class="chart-card">
                        <h3>{title}</h3>
                        <img src="{img_prefix}{png_filename}" alt="{title}" loading="lazy">
                    </div>
'''
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Generate CB Speeches dashboard')
    ...
    # Determine if using relative paths (for GitHub Pages)
    use_relative = 'docs' in str(output_path)
    ...
    html = generate_html(data_summary, use_relative_paths=use_relative)
